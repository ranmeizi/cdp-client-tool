...
- `src/socket-io-device.gateway.ts`：实现 `DeviceGateway`，维护设备 Map，通过 Socket 事件转发 `read_dir` / `read_file` / `rm` 等。

客户端（如 examples/client）连接时需带上 `deviceName`（如 query 或 auth），网关据此区分设备并响应 HTTP 请求。
